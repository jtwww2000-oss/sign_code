...
module sign_y_ntt_mac_intt #(
    ...
    always @(posedge clk) begin
        ...
        // 巧妙利用了 S_COPY_STORE 和 S_OUT_STORE 这两个状态
        // 它们恰好在每次新的矩阵行开始计算前发生，并且 general_cnt 会遍历 0 到 255
        // 这实现了对 acc_bram 的完美 0 时钟开销清零！
    ...
    always @(*) begin
        ...
        if (state == S_COPY_C && general_cnt > 0) begin
            c_post_we = 1'b1; c_post_addr_w = general_cnt[7:0] - 8'd1; c_post_wdata = ntt_ram0_rdata_a;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_COPY_STORE: begin
                    y_post_we <= 1'b1; y_post_addr_w <= {l_cnt[1:0], general_cnt[7:0]}; y_post_wdata <= ntt_ram0_rdata_a; 
                ...
                S_OUT_STORE: begin
                    o_w_valid <= 1'b1; o_w_data <= intt_ram0_rdata_a; o_w1_data <= w1_calc; 
                    o_w_poly_idx <= k_cnt; o_w_coeff_idx <= general_cnt[7:0];
                    w_we <= 1'b1; w_addr_w <= {k_cnt[1:0], general_cnt[7:0]}; w_wdata <= intt_ram0_rdata_a;
                ...
                S_CS2_OUT: begin
                    ...
                    if (out_valid_d1) begin
                        o_cs2_valid <= 1'b1; o_cs2_data <= intt_ram0_rdata_a;
                ...
                S_CT0_OUT: begin
                    ...
                    if (out_valid_d1) begin
                        o_ct0_valid <= 1'b1; o_ct0_data <= intt_ram0_rdata_a;
