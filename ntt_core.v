...
module ntt_core #(
    ...
    reg [5:0] pre_sr_val;         
    reg [7:0] pre_sr_addr [0:5];  
    reg [7:0] pre_rom_addr_reg;   

    // 【核心修复】：位反转流水线加深为 2 级，对齐 RAM 读延迟
    reg [1:0] br_sr_val;
    reg [7:0] br_sr_addr [0:1];

    reg [6:0] sr_val;       
    reg [6:0] sr_ping_pong; 
    reg [7:0] sr_addr_u [0:6];
    reg [7:0] sr_addr_v [0:6];
    reg [WIDTH-1:0] u_d1, u_d2, u_d3, u_d4; 
    ...
    mod_add #( .WIDTH(WIDTH) ) u_mod_add ( .a(u_d4), .b(mont_res), .q(Q), .res(mod_add_out) );
    mod_sub #( .WIDTH(WIDTH) ) u_mod_sub ( .a(u_d4), .b(mont_res), .q(Q), .res(mod_sub_out) );
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            u_d1 <= 0; u_d2 <= 0; u_d3 <= 0; u_d4 <= 0;
            res_u_reg <= 0; res_v_reg <= 0;
            
            for(i=0; i<6; i=i+1) pre_sr_addr[i] <= 0;
            for(i=0; i<7; i=i+1) begin sr_addr_u[i] <= 0; sr_addr_v[i] <= 0; end
        end else begin
            ram0_we_a <= 0; ram0_we_b <= 0; ram1_we_a <= 0; ram1_we_b <= 0;
        
            // ================= 流水线移位逻辑 =================
            // 【核心修复】：位反转的地址和使能，推迟 1 拍，保证写回时 RAM 已经吐出正确数据
            ...
            if (state == S_PRE_RUN || state == S_PRE_FLUSH) begin
                pre_sr_val <= {pre_sr_val[4:0], pipe_en};
                pre_sr_addr[0] <= cnt[7:0];
                for (i=1; i<6; i=i+1) pre_sr_addr[i] <= pre_sr_addr[i-1];
            end

            if (state == S_NTT_RUN || state == S_NTT_FLUSH) begin
                sr_val <= {sr_val[5:0], pipe_en};  
                sr_ping_pong <= {sr_ping_pong[5:0], stage_cnt[0]};
                
                sr_addr_u[0] <= addr_u_gen; sr_addr_v[0] <= addr_v_gen;
                for (i=1; i<7; i=i+1) begin
                    sr_addr_u[i] <= sr_addr_u[i-1]; sr_addr_v[i] <= sr_addr_v[i-1];
                end
                
                u_d1 <= current_u; u_d2 <= u_d1; u_d3 <= u_d2; u_d4 <= u_d3;
            ...
            case (state)
                ...
                S_PRE_RUN: begin
                    ...
                    if (pre_sr_val[5]) begin
                        ram1_we_a <= 1; ram1_addr_a <= pre_sr_addr[5]; ram1_wdata_a <= mont_res; 
                    end
                end
                S_PRE_FLUSH: begin
                    if (pre_sr_val[5]) begin
                ...
                S_NTT_RUN: begin
                    ...
                    if (sr_val[6]) begin
                        if (sr_ping_pong[6] == 0) begin 
                            ram1_we_a <= 1; ram1_addr_a <= sr_addr_u[6]; ram1_wdata_a <= res_u_reg;
                            ram1_we_b <= 1; ram1_addr_b <= sr_addr_v[6]; ram1_wdata_b <= res_v_reg;
                        end else begin                  
                            ram0_we_a <= 1; ram0_addr_a <= sr_addr_u[6]; ram0_wdata_a <= res_u_reg;
                            ram0_we_b <= 1; ram0_addr_b <= sr_addr_v[6]; ram0_wdata_b <= res_v_reg;
                        end
                    end
                end
                S_NTT_FLUSH: begin
